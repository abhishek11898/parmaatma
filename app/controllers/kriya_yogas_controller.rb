class KriyaYogasController < ApplicationController
  ...
  def new 
    @kriya_yoga = KriyaYoga.new
  end

  def create
    @kriya_yoga = KriyaYoga.new(kriya_yoga_params)
    if params[:kriya_yoga][:image_name].present?
      ...
      image_new_filename = "#{KriyaYoga.count + 1}#{image_extension}" # Use KriyaYoga.count to ensure unique filenames
    ...
    if params[:kriya_yoga][:video].present?
      ...
      video_new_filename = "#{KriyaYoga.count + 1}#{video_extension}" # Use KriyaYoga.count to ensure unique filenames
  ...
  def update
    ...
    if @kriya_yoga.update(kriya_yoga_params)
      ...
      flash[:error] = 'Unable to update Kriya yoga item.'
      render :edit
    end
  end
  ...
  def destroy
    @kriya_yoga = KriyaYoga.find(params[:id])
    if @kriya_yoga.present?
      image_path = @kriya_yoga.absolute_image_url
      video_path = @kriya_yoga.absolute_video_url
      ...
      redirect_to controller: 'kriya_yogas', action: :index
    else
  ...
  def kriya_yoga_params
    params.require(:kriya_yoga).permit(:title, :description, :reference_name)
  end
end